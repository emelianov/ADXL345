...
void ADXL345::readAccel(int16_t *x, int16_t *y, int16_t *z) {
	...
	// each axis reading comes in 10 bit resolution, ie 2 bytes.  Least Significat int8_t first!!
...
void ADXL345::writeTo(int8_t address, int8_t val) {
...
void ADXL345::readFrom(int8_t address, int16_t num, int8_t _buff[]) {
	Wire.beginTransmission(ADXL345_DEVICE); // start transmission to device 
...
void ADXL345::getRangeSetting(int8_t* rangeSetting) {
	int8_t _b;
...
void ADXL345::setAxisOffset(int16_t x, int16_t y, int16_t z) {
	writeTo(ADXL345_OFSX, int8_t (x));  
	writeTo(ADXL345_OFSY, int8_t (y));  
	writeTo(ADXL345_OFSZ, int8_t (z));  
}
...
// Sets the DUR byte
// The DUR int8_t contains an unsigned time value representing the maximum time
...
void ADXL345::setDoubleTapLatency(int16_t doubleTapLatency) {
	int8_t _b = int8_t (doubleTapLatency);
...
void ADXL345::setDoubleTapWindow(int16_t doubleTapWindow) {
	...
	int8_t _b = int8_t (doubleTapWindow);
...
// Sets the THRESH_ACT int8_t which holds the threshold value for detecting activity.
...
void ADXL345::setActivityThreshold(int16_t activityThreshold) {
	...
	int8_t _b = int8_t (activityThreshold);
...
// Sets the THRESH_INACT int8_t which holds the threshold value for detecting inactivity.
...
void ADXL345::setInactivityThreshold(int16_t inactivityThreshold) {
	...
	int8_t _b = int8_t (inactivityThreshold);
...
void ADXL345::setTimeInactivity(int16_t timeInactivity) {
	...
	int8_t _b = int8_t (timeInactivity);
...
void ADXL345::setFreeFallThreshold(int16_t freeFallThreshold) {
	...
	int8_t _b = int8_t (freeFallThreshold);
...
void ADXL345::setFreeFallDuration(int16_t freeFallDuration) {
	...
	int8_t _b = int8_t (freeFallDuration);
...
void ADXL345::set_bw(int8_t bw_code){
...
bool ADXL345::triggered(int8_t interrupts, int16_t mask){
...
int8_t ADXL345::getInterruptSource() {
	int8_t _b;
	readFrom(ADXL345_INT_SOURCE, 1, &_b);
	return _b;
}

bool ADXL345::getInterruptSource(int8_t interruptBit) {
	return getRegisterBit(ADXL345_INT_SOURCE,interruptBit);
}

bool ADXL345::getInterruptMapping(int8_t interruptBit) {
	return getRegisterBit(ADXL345_INT_MAP,interruptBit);
}

// Set the mapping of an interrupt to pin1 or pin2
// eg: setInterruptMapping(ADXL345_INT_DOUBLE_TAP_BIT,ADXL345_INT2_PIN);
void ADXL345::setInterruptMapping(int8_t interruptBit, bool interruptPin) {
	setRegisterBit(ADXL345_INT_MAP, interruptBit, interruptPin);
}

bool ADXL345::isInterruptEnabled(int8_t interruptBit) {
	return getRegisterBit(ADXL345_INT_ENABLE,interruptBit);
}

void ADXL345::setInterrupt(int8_t interruptBit, bool state) {
	setRegisterBit(ADXL345_INT_ENABLE, interruptBit, state);
}

void ADXL345::setRegisterBit(int8_t regAdress, int16_t bitPos, bool state) {
	int8_t _b;
...
bool ADXL345::getRegisterBit(int8_t regAdress, int16_t bitPos) {
	int8_t _b;
	readFrom(regAdress, 1, &_b);
	return ((_b >> bitPos) & 1);
}

// print16_t all register value to the serial ouptut, which requires it to be setup
