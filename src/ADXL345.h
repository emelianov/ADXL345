 ...
 *                                                                         *
 ***************************************************************************
   Originalylibrary was taken this from
...
public:
	...
	int8_t error_code;       // Initial state
	...
	void set_bw(int8_t bw_code);
	int8_t get_bw_code();  
	
	
	bool triggered(int8_t interrupts, int16_t mask);
	
	
	int8_t getInterruptSource();
	bool getInterruptSource(int8_t interruptBit);
	bool getInterruptMapping(int8_t interruptBit);
	void setInterruptMapping(int8_t interruptBit, bool interruptPin);
	bool isInterruptEnabled(int8_t interruptBit);
	void setInterrupt(int8_t interruptBit, bool state);
	
	void getRangeSetting(int8_t* rangeSetting);
...
private:
	void writeTo(int8_t address, int8_t val);
	void readFrom(int8_t address, int16_t num, int8_t buff[]);
	void setRegisterBit(int8_t regAdress, int16_t bitPos, bool state);
	bool getRegisterBit(int8_t regAdress, int16_t bitPos);  
	int8_t _buff[6] ;    //6 bytes buffer for saving data read from the device
};
